...
/**
 * Selected Phase 2 source metadata cross-check (moved here from
 * content-validator by the validation-consolidation slice; behavior is
 * verbatim).
 *
 * Unlike validateSourceTransparencyCollection above, this audit's value is
 * CROSS-surface identity: runtime catalog vs build-time manifest vs extracted
 * documents vs the human-authored SOURCES.md rows. The shared collection
 * boundary validates one collection against the repository projection and
 * cannot express these joins, so this remains an audit-time check rather than
 * production wiring.
 */
