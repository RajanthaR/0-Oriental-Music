...
|---|---|---|---|
| `C3-01`–`C3-08` | Status, identity, evidence, summary, batch, and projection decisions could diverge or reread mutable values. | One bounded operation context, checked batch, complete kind contract, fresh summary, and allowlisted projection fail closed. | `src/lib/data/publication-policy.ts:386 (createPublicationEvaluationContext)`, `src/lib/data/publication-policy.ts:1375 (evaluatePublicationBatch)`, `src/lib/data/repository.ts:507 (getPublicationSummary)`, `src/lib/validation/content-contracts.ts:985 (projectPublicRecord)`. |
...
|---|---|---|
| `C3-V01`, `C3-V04` | Normalize quarantine identities and expose only the immutable predicate. `src/lib/data/publication-policy.ts:130 (isKnownQuarantinedEntityId)`; `src/test/publication-containment.test.ts:130`. | **FIXED-PENDING-ACCEPTANCE-REVIEW** |
| `C3-V02`, `C3-V03` | Reject malformed source-page metadata and withhold Tala dispositions from unsafe contexts. `src/lib/data/publication-policy.ts:316 (isSourceDocumentRecord)`; `src/lib/data/publication-policy.ts:326 (isSourcePageQualityRecord)`; `src/lib/data/publication-policy.ts:923 (getTalaFieldDisposition)`. | **FIXED-PENDING-ACCEPTANCE-REVIEW** |
| `C3-V05` | QuizRunner validates and projects one detached quiz/question snapshot. `src/components/quiz/QuizRunner.tsx:37 (getUsableQuiz)`; `src/test/quiz-runner.test.tsx:175`. | **FIXED-PENDING-ACCEPTANCE-REVIEW** |
| `C3-V06`, `C3-V07` | Public collection validators enforce the declared kind and padded source IDs fail consistently. `src/lib/validation/content-validator.ts:615 (validatePublicCollection)`; `src/test/publication-containment.test.ts:699`; `src/test/publication-containment.test.ts:706`. | **FIXED-PENDING-ACCEPTANCE-REVIEW** |
| `C3-V08` | Sinhala identity/search normalization removes U+2060–U+206F controls. `src/lib/search/normalize-sinhala.ts:14`; `src/test/search-engine.test.ts:12`. | **FIXED-PENDING-ACCEPTANCE-REVIEW** |
| `C3-V09` | Every dependency key now reaches repository list, direct lookup, search-catalog where applicable, and summary assertions. `src/test/publication-parity.test.ts:332`. | **FIXED-PENDING-ACCEPTANCE-REVIEW** |
| `C3-V10` | Numeric anchors resolve at the exact cited line and the six-column closeout table mirrors the ledger. `src/test/review-closeout.test.ts:64 (expectSemanticReference)`; `src/test/review-closeout.test.ts:136`. | **FIXED-PENDING-ACCEPTANCE-REVIEW** |
| `C3-V11` | Rhythm callback replacement regression completes the active session and proves only the latest callback fires. `src/test/components.test.tsx:891`. | **FIXED-PENDING-ACCEPTANCE-REVIEW** |
| `C3-V12` | Microphone privacy coverage guards fetch, XHR, WebSocket, and beacon on successful and failed starts. `src/test/pitch.test.ts:389`. | **FIXED-PENDING-ACCEPTANCE-REVIEW** |
| `C3-V13`, `C3-V14` | AudioContext mocks restore in `finally`; rejected CMS publication no longer performs a mutating reset. `src/test/synth.test.ts:57`; `src/test/publication-containment.test.ts:143`. | **FIXED-PENDING-ACCEPTANCE-REVIEW** |
