...
|---|---|---|---|
| `C3-01`, `C3-04`–`C3-08` | Publication status drift, hostile batches, decision/projection TOCTOU, duplicate IDs, mismatched kinds, and stale identity catalogs. | Per-operation bounded snapshots, checked batches, complete kind contracts, and allowlisted projections fail closed. Summary and repository consumers use fresh decisions. | `src/lib/data/publication-policy.ts:386 (createPublicationEvaluationContext)`, `src/lib/data/publication-policy.ts:1375 (evaluatePublicationBatch)`, `src/lib/validation/content-contracts.ts:788 (validateContentRecord)`, `src/lib/validation/content-contracts.ts:985 (projectPublicRecord)`. |
...
| `P02-PITCH-OWNERSHIP-001` | Partial or stale microphone starts could retain MediaStreams, AudioContexts, nodes, animation frames, or callbacks. | Latest-generation ownership and idempotent cleanup stop late streams and reclaim every partial resource; audio remains local-only. | `src/lib/audio/pitch.ts:40 (generation ownership)`, `src/lib/audio/pitch.ts:114 (startListening)`, `src/lib/audio/pitch.ts:220 (stopListening)`, `src/test/pitch.test.ts:230 (late stream cleanup)`, `src/test/pitch.test.ts:247 (newest pending start)`. |
...
| `C3-03` | `validator-03.json` | `src/test/source-metadata-consistency.test.ts:18 (unknown provenance representation)` | `src/lib/validation/content-contracts.ts#projectPublicRecord` | `src/lib/validation/content-contracts.ts:985 (projectPublicRecord)` | **FIXED-PENDING-REREVIEW** |
| `C3-04` | `validator-04.json` | `src/test/publication-containment.test.ts:201 (duplicate IDs)` | `src/lib/data/publication-policy.ts#evaluatePublicationBatch` | `src/lib/data/publication-policy.ts:1375 (evaluatePublicationBatch)` | **FIXED-PENDING-REREVIEW** |
| `C3-05` | `validator-05.json` | `src/test/content-contracts.test.ts:475 (detached snapshot)` | `src/lib/data/publication-policy.ts#captureEvaluationValue` | `src/lib/data/publication-policy.ts:289 (captureEvaluationValue)` | **FIXED-PENDING-REREVIEW** |
...
| `C3-07` | `validator-07.json` | `src/test/content-contracts.test.ts:342 (unknown or ambiguous kinds)` | `src/lib/validation/content-contracts.ts#projectPublicRecord` | `src/lib/validation/content-contracts.ts:985 (projectPublicRecord)` | **FIXED-PENDING-REREVIEW** |
| `C3-08` | `validator-08.json` | `src/test/publication-containment.test.ts:566 (identity containment)` | `src/lib/data/publication-policy.ts#createPublicationEvaluationContext` | `src/lib/data/publication-policy.ts:386 (createPublicationEvaluationContext)` | **FIXED-PENDING-REREVIEW** |
...
| `C3-10` | `validator-10.json` | `src/test/source-metadata-consistency.test.ts:79 (ambiguous extracted-document mappings)` | `src/lib/validation/content-validator.ts#validateSelectedSourceMetadata` | `src/lib/validation/content-validator.ts:229 (validateSelectedSourceMetadata)` | **FIXED-PENDING-REREVIEW** |
| `C3-11` | `validator-11.json` | `src/test/publication-containment.test.ts:59 (declarative %s dependency rule)` | `src/lib/data/publication-policy.ts#DEPENDENCY_FIELD_RULES` | `src/lib/data/publication-policy.ts:225 (DEPENDENCY_FIELD_RULES)` | **FIXED-PENDING-REREVIEW** |
| `C3-12` | `validator-12.json` | `src/test/source-metadata-consistency.test.ts:79 (ambiguous extracted-document mappings)` | `src/lib/validation/content-validator.ts#validateSelectedSourceMetadata` | `src/lib/validation/content-validator.ts:229 (validateSelectedSourceMetadata)` | **FIXED-PENDING-REREVIEW** |
...
| `C3-14` | `validator-14.json` | `src/test/content-contracts.test.ts:384 (discriminator-compatible fields)` | `src/lib/validation/content-contracts.ts#projectPublicRecord` | `src/lib/validation/content-contracts.ts:985 (projectPublicRecord)` | **FIXED-PENDING-REREVIEW** |
| `C3-15` | `validator-15.json` | `src/test/quiz-runner.test.tsx:198 (duplicate question IDs)` | `src/components/quiz/QuizRunner.tsx#getUsableQuiz` | `src/components/quiz/QuizRunner.tsx:37 (getUsableQuiz)` | **FIXED-PENDING-REREVIEW** |
| `C3-16` | `validator-16.json` | `src/test/search-engine.test.ts:83 (normalized-empty controls)` | `src/lib/search/search-engine.ts#classifySearchQuery` | `src/lib/search/search-engine.ts:84 (classifySearchQuery)` | **FIXED-PENDING-REREVIEW** |
| `C3-17` | `validator-17.json` | `src/test/components.test.tsx:891 (rhythm session)` | `src/components/audio/RhythmTapGame.tsx#RhythmTapGame` | `src/components/audio/RhythmTapGame.tsx:8 (RhythmTapGame)` | **FIXED-PENDING-REREVIEW** |
| `C3-18` | `validator-18.json` | `src/test/components.test.tsx:913 (completed rhythm playback handles)` | `src/components/audio/TalaVisualizer.tsx#TalaVisualizer` | `src/components/audio/TalaVisualizer.tsx:9 (TalaVisualizer)` | **FIXED-PENDING-REREVIEW** |
| `C3-19` | `validator-19.json` | `src/test/components.test.tsx:935 (stale Tala unavailable callbacks)` | `src/components/audio/TalaVisualizer.tsx#TalaVisualizer` | `src/components/audio/TalaVisualizer.tsx:9 (TalaVisualizer)` | **FIXED-PENDING-REREVIEW** |
| `C3-20` | `validator-20.json` | `src/test/components.test.tsx:1003 (Tabla promises reject)` | `src/lib/audio/tabla.ts#scheduleTablaPlan` | `src/lib/audio/tabla.ts:46 (scheduleTablaPlan)` | **FIXED-PENDING-REREVIEW** |
| `P02-PITCH-OWNERSHIP-001` | `metadata.json authorizedAdditionalFindings` | `src/test/pitch.test.ts:230 (late stream)` | `src/lib/audio/pitch.ts#startListening` | `src/lib/audio/pitch.ts:114 (startListening)` | **FIXED-PENDING-REREVIEW** |
| `P02-PROJECT-SCOPE-001` | `metadata.json authorizedAdditionalFindings` | `src/test/review-closeout.test.ts#records the acceptance-hardening scope` | `AGENTS.md#current verified public curriculum boundary` | `AGENTS.md:3 (current verified public curriculum boundary)` | **FIXED-PENDING-REREVIEW** |
