...
- **Skill review:** Every phase invokes the full `rajantha-skills-library:ce-code-review` pipeline against its recorded base. Always-on and relevant conditional reviewers inspect the complete diff, surviving findings receive independent validation, safe fixes land in local review commits, and auditable run artifacts preserve coverage and verdict evidence.
...
- **G5 — Review:** The full `ce-code-review` run covers the complete phase diff, required reviewers and validators complete, artifacts are preserved, the verdict is `Ready to merge`, no actionable finding or degraded P0/P1 validation remains, and explicit-plan requirements are addressed.
...
**Acceptance:** No unsupported scope is publicly discoverable; quarantined direct routes are safe; counts and states are data-derived; known issues are ledgered rather than erased; final gates and the mandatory skill-review acceptance criteria pass.
...
- **P7-U07 Final skill review:** Run the full `ce-code-review` workflow across all Phase 7 changes and the declared high-risk cross-phase audit scope, validate and fix findings through local review commits, preserve artifacts, and open the final ready PR only after a `Ready to merge` verdict.
...
- the review skill, required reviewers, validators, or artifacts remain unavailable/degraded, or actionable findings remain after three review-fix cycles;
